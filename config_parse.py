...
def parse_pox_param_file(pox_config_file):
    ...
    assert(f_line_contents is not None)
    # maybe not the most efficient/clean/etc. way to do this, but should only have to be done once so NBD
    m_mat = np.asarray([np.fromstring(line.rstrip(),dtype=float,sep=' ') for line in f_line_contents[1:5]])
    screw_vec = np.asarray([np.fromstring(line.rstrip(),dtype=float,sep=' ') for line in f_line_contents[6:]])
